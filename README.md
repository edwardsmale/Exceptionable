...
```C#
public Exceptionable<string> MethodThatReturnsExceptionable(int positiveNumber)
{
    if (positiveNumber > 0)
    {
...
```C#
var result = MethodThatReturnsExceptionable(positiveNumber: 42);
...
```C#
var result = MethodThatReturnsExceptionable(positiveNumber: -1);

Assert(result.HasException == true);
...
```C#
var text = new Exceptionable<string>("A piece of text");

var result = MethodThatTakesAString(text);
...
```C#
var value = new Exceptionable<int>(3);
